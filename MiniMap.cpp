...
void MiniMap::Render() {
	iexPolygon::Rect(0,540 - Limits.y, Limits.x,Limits.y, 0, 0x5F000000, 0);
	iexPolygon::Rect(p_Pps.x - Player_Icon_OFSET, 535 - Limits.y + p_Pps.y + Player_Icon_OFSET, Player_Icon_SIZE, Player_Icon_SIZE, 0, 0xFFFF0000);
	for (int i = 0; i < count; i++) {
		iexPolygon::Rect(E_Pps[i].x- Enemy_Icon_SIZE, 540 - Limits.y + E_Pps[i].y - Enemy_Icon_SIZE, Enemy_Icon_SIZE, Enemy_Icon_SIZE, 0, 0xFFFFFF00);
	}
