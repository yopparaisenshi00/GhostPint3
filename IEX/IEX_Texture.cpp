...
Texture2D* iexTexture::Load( char* filename, int flag )
{
	...
	if( no == -1 ){
		...
		//	テクスチャ読み込み
		char	str[8];
		CopyMemory(str,filename+strlen(filename)-4,5);
		for (int i=0; i<4; i++)	str[i]	= toupper(str[i]);
		if (lstrcmp(str,".BMP") == 0)
		{
			//	BMP読み込み(透過色対応)
			hr = D3DXCreateTextureFromFileEx(
				iexSystem::GetDevice(),filename,0,0,0,0,
				fmt,D3DPOOL_MANAGED,D3DX_FILTER_LINEAR,D3DX_FILTER_LINEAR,
				TRANSPARENT_COLOR,						// 透過色とする
				NULL,NULL,&TexInfo[no].lpTexture
			);
		}
		else
		{
			//	BMP以外読み込み
			hr = D3DXCreateTextureFromFileEx(
				iexSystem::GetDevice(),filename,D3DX_DEFAULT,D3DX_DEFAULT,1,0,
				fmt,D3DPOOL_DEFAULT,D3DX_FILTER_POINT,D3DX_FILTER_POINT,
				0x00000000,
				NULL,NULL,&TexInfo[no].lpTexture
			);
		}
