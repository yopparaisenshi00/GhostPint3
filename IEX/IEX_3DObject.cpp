...
typedef struct tagIEMBONE1 {
	...
	WORD			IndexNum;			//	影響頂点数
	DWORD			Index[1000];		//	影響頂点Index
	float			Influence[1000];	//	影響力
} IEMBONE1, *LPIEMBONE1;

typedef struct tagIEMMOTION1 {
	WORD			NumRotate;			//	回転キーフレーム数
	WORD			RotateFrame[512];	//	回転キーフレーム
	Quaternion		Rotate[512];		//	ボーンの状態クォータニオン

	WORD			NumPosition;		//	座標キーフレーム数
	WORD			PositionFrame[512];	//	座標キーフレーム
...
typedef struct tagIEMBONE {
	...
	WORD			IndexNum;			//	影響頂点数
	DWORD*			Index;				//	影響頂点Index
	float*			Influence;			//	影響力
} IEMBONE, *LPIEMBONE;

typedef struct tagIEMMOTION {
	WORD			NumRotate;			//	回転キーフレーム数
	WORD*			RotateFrame;		//	回転キーフレーム
	Quaternion*		Rotate;				//	ボーンの状態クォータニオン

	WORD			NumPosition;		//	座標キーフレーム数
	WORD*			PositionFrame;		//	座標キーフレーム
	Vector3*		Position;			//	座標
...
struct tagIEMFILE
{
	...
	WORD			NumFace;			//	ポリゴン数
	WORD*			lpFace;				//	ポリゴンインデックス
	DWORD*			lpAtr;				//	ポリゴン材質

	WORD			NumMaterial;		//	マテリアル数
